    ...
    fn maybe_relocate_bar(&mut self) {
                ...
                .move_range(old_base, new_base, CAPABILITY_BAR_SIZE)
        {
            ...
            if let Err(err) = self.set_notification_ioevents(&self.vm, new_base, false) {
                ...
                // Keep the range allocated if ioeventfds are still registered
                // in it, otherwise another device could get it and fail to
                // register its own ioeventfds.
            } else {
                let _ = self.vm.resource_allocator().mmio64_memory.free(&new_range);
            }
            return;
...
mod tests {
    ...
    use super::{KvmVm, PciCapabilityType, VirtioPciDevice};
    use crate::Vmm;
